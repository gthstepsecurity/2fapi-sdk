...
///
/// Uses variable-time multiscalar multiplication for efficiency.
/// This is safe because points G and H are public generators.
pub fn commit(secret: &Scalar, blinding: &Scalar) -> RistrettoPoint {
    let g = generators::generator_g();
    let h = generators::generator_h();
    RistrettoPoint::vartime_multiscalar_mul([secret, blinding], [g, h])
}

/// Computes a Pedersen commitment using explicit generators.
///
/// C = s·G + r·H where G and H are provided by the caller.
pub fn commit_with_generators(
    ...
    RistrettoPoint::vartime_multiscalar_mul([secret, blinding], [*generator_g, *generator_h])
}
